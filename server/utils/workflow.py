...
class WorkFlowAPiRequest(object):
    def __init__(self, token=settings.WORKFLOW_TOKEN, appname=settings.WORKFLOW_APP, username='admin',
    ...
    def getdata(self, parameters=dict(), method='get', url='/api/v1.0/workflows/', timeout=300, data=dict()):
        ...
        try:
            r = getattr(requests, method)('{0}{1}'.format(self.workflowurl, url), headers=headers, params=parameters,
