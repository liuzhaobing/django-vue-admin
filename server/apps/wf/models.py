...
class Workflow(CommonAModel):
    ...
    limit_expression = models.JSONField('限制表达式', default=dict, blank=True,
    ...
    title_template = models.CharField('标题模板', max_length=50, default='{title}', null=True, blank=True,
                                      help_text='工单字段的值可以作为参数写到模板中，格式如：你有一个待办工单:{title}')
    content_template = models.CharField('内容模板', max_length=1000, default='标题:{title}, 创建时间:{create_time}',
...
class State(CommonAModel):
    ...
    sort = models.IntegerField('状态顺序', default=0,
    ...
    enable_retreat = models.BooleanField('允许撤回', default=False,
    ...
    state_fields = models.JSONField('表单字段', default=dict,
                                    help_text='json格式字典存储,包括读写属性1：只读，2：必填，3：可选, 4:隐藏 示例：{"create_time":1,"title":2, "sn":1}, 内置特殊字段participant_info.participant_name:当前处理人信息(部门名称、角色名称)，state.state_name:当前状态的状态名,workflow.workflow_name:工作流名称')  # json格式存储,包括读写属性1：只读，2：必填，3：可选，4：不显示, 字典的字典
...
class Transition(CommonAModel):
    ...
    workflow = models.ForeignKey(Workflow, on_delete=models.CASCADE, verbose_name='所属工作流')
    timer = models.IntegerField('定时器(单位秒)', default=0,
                                help_text='单位秒。处于源状态X秒后如果状态都没有过变化则自动流转到目标状态。设置时间有效')
    source_state = models.ForeignKey(State, on_delete=models.CASCADE, verbose_name='源状态',
                                     related_name='sstate_transition')
    destination_state = models.ForeignKey(State, on_delete=models.CASCADE, verbose_name='目的状态',
    ...
    attribute_type = models.IntegerField('属性类型', default=1, choices=attribute_type_choices,
...
class CustomField(CommonAModel):
    ...
    placeholder = models.CharField('占位符', max_length=100, blank=True, null=True,
                                   help_text='用户工单详情表单中作为字段的占位符显示')
    field_template = models.TextField('文本域模板', null=True, blank=True,
    ...
    label = models.CharField('标签', max_length=1000, default='',
...
class Ticket(CommonBModel):
    ...
    parent_state = models.ForeignKey(State, null=True, blank=True, on_delete=models.CASCADE, verbose_name='父工单状态',
    ...
    add_node_man = models.ForeignKey(User, verbose_name='加签人', on_delete=models.SET_NULL, null=True, blank=True,
                                     help_text='加签操作的人，工单当前处理人处理完成后会回到该处理人，当处于加签状态下才有效')
    script_run_last_result = models.BooleanField('脚本最后一次执行结果', default=True)
    participant_type = models.IntegerField('当前处理人类型', default=0, help_text='0.无处理人,1.个人,2.多人',
                                           choices=State.state_participanttype_choices)
    participant = models.JSONField('当前处理人', default=list, blank=True,
    ...
    multi_all_person = models.JSONField('全部处理的结果', default=dict, blank=True,
...
class TicketFlow(BaseModel):
    ...
    participant_type = models.IntegerField('处理人类型', default=0, help_text='0.无处理人,1.个人,2.多人等',
                                           choices=State.state_participanttype_choices)
    ...
    participant_str = models.CharField('处理人', max_length=200, null=True, blank=True,
    ...
    intervene_type = models.IntegerField('干预类型', default=0, help_text='流转类型',
