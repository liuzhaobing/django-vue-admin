...
class TicketListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ticket
        fields = ['id', 'title', 'sn', 'workflow', 'workflow_', 'state', 'state_', 'act_state', 'create_time',
                  'update_time', 'participant_type', 'create_by']
                ...
                if 'sys_user' in i['label']:
                    if isinstance(i['field_value'], list):
                        i['field_display'] = ','.join(
                            list(User.objects.filter(id__in=i['field_value']).values_list('name', flat=True)))
                    else:
                        i['field_display'] = User.objects.get(id=i['field_value']).name
                elif 'deptSelect' in i['label']:
                    if isinstance(i['field_value'], list):
                        i['field_display'] = ','.join(
                            list(Organization.objects.filter(id__in=i['field_value']).values_list('name', flat=True)))
                    else:
...
class TicketDestorySerializer(serializers.Serializer):
    ids = serializers.ListField(child=serializers.PrimaryKeyRelatedField(queryset=Ticket.objects.all()),
                                label='工单ID列表')
