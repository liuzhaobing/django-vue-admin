...
class WfService(object):
    ...
    def get_workflow_custom_fields_list(workflow: Workflow):
        ...
        """
        return list(
            CustomField.objects.filter(is_deleted=False, workflow=workflow).order_by('sort').values_list('field_key',
                                                                                                         flat=True))
    ...
    def get_ticket_sn(cls, workflow: Workflow):
        ...
        ticket_day_count_new = Ticket.objects.filter(create_time__gte=today, create_time__lte=next_day,
                                                     workflow=workflow).count() + 1
        return '%s_%04d%02d%02d%04d' % (workflow.sn_prefix, now.year, now.month, now.day, ticket_day_count_new)

    @classmethod
    def get_next_state_by_transition_and_ticket_info(cls, ticket: Ticket, transition: Transition,
                                                     new_ticket_data: dict = {}) -> object:
        """
        获取下个节点状态
        """
    ...
    @classmethod
    def get_ticket_state_participant_info(cls, state: State, ticket: Ticket, new_ticket_data: dict = {},
                                          handler: User = None):
        """
        ...
        if destination_participant_type == State.PARTICIPANT_TYPE_FIELD:
            destination_participant = new_ticket_data.get(destination_participant,
                                                          0) if destination_participant in new_ticket_data \
            ...
            destination_participant = list(
                User.objects.filter(dept__in=destination_participant).values_list('id', flat=True))
    ...
    def ticket_handle_permission_check(cls, ticket: Ticket, user: User) -> dict:
        ...
        elif participant_type in [State.PARTICIPANT_TYPE_MULTI, State.PARTICIPANT_TYPE_DEPT,
    ...
    def handle_ticket(cls, ticket: Ticket, transition: Transition, new_ticket_data: dict = {}, handler: User = None,
                      suggestion: str = '', created: bool = False, by_timer: bool = False, by_task: bool = False,
                      by_hook: bool = False):
        ...
        if multi_all_person:
            ...
            if WfService.check_dict_has_all_same_value(multi_all_person):
                participant_info = WfService.get_ticket_state_participant_info(destination_state, ticket,
                                                                               new_ticket_data)
