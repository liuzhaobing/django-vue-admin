    ...
    <el-card style="margin-top: 2px">
      ...
      >
        ...
        <el-table-column width="180" label="类型">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.type==0" effect="plain">
              普通类型
            </el-tag>
            <el-tag v-if="scope.row.type==1" effect="plain">
              初始状态
            </el-tag>
            <el-tag v-if="scope.row.type==2" effect="plain">
              结束状态
            </el-tag>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item
          ...
          label-width="120px"
        >
          <el-switch v-model="wfstate.is_hidden" />
        </el-form-item>
        ...
        <el-form-item label="状态类型" prop="type">
          ...
          <el-select v-model="wfstate.type" style="width: 100%" placeholder="请选择">
            <el-option
        ...
        <el-form-item
          ...
          label-width="120px"
        >
          <el-switch v-model="wfstate.enable_retreat" />
        </el-form-item>
        <el-form-item label="参与者类型" prop="participant_type">
          <el-select v-model="wfstate.participant_type" style="width: 100%" placeholder="请选择" @change="typeChange">
            <el-option
        ...
        <el-form-item v-if="wfstate.participant_type==1" label="参与者" prop="participant">
          <el-select v-model="participant" style="width: 100%" placeholder="请选择参与者">
            <el-option v-for="item in staffs" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="wfstate.participant_type==2" label="参与者" prop="participant">
          <el-select v-model="participants" style="width: 100%" multiple placeholder="请选择参与者">
            <el-option v-for="item in staffs" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="wfstate.participant_type==7" label="参与者" prop="participant">
          <el-select v-model="participant" style="width: 100%" placeholder="请选择字段">
            <el-option
        ...
        <el-form-item v-if="wfstate.participant_type==2" label="分配方式" prop="participant">
          <el-select v-model="wfstate.distribute_type" style="width: 100%" placeholder="请选择分配方式">
            <el-option label="主动接单" value="1" />
            <el-option label="直接处理" value="2" />
            <el-option label="随机分配" value="3" />
            <el-option label="全部处理" value="4" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="wfstate.participant_type==4" label="角色" prop="participant">
          <el-select v-model="participants" style="width: 100%" multiple placeholder="请选择角色">
            <el-option v-for="item in roles" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="wfstate.participant_type==4" label="过滤策略" prop="participant">
          <el-select v-model="wfstate.filter_policy" style="width: 100%" placeholder="请选择过滤策略">
            <el-option label="无" value="0" />
            <el-option label="和工单同属以及上级部门" value="1" />
            <el-option label="和创建人同属以及上级部门" value="2" />
            <el-option label="和上步处理人同属以及上级部门" value="3" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="wfstate.participant_type==9" label="代码选择" prop="participant">
          <el-select v-model="participants" style="width: 100%" placeholder="请选择代码">
            <el-option v-for="item in codes" :key="item.func" :label="item.name" :value="item.func" />
          </el-select>
        </el-form-item>
        <el-form-item label="抄送给" prop="participant">
          <el-select v-model="wfstate.participant_cc" style="width: 100%" multiple placeholder="请选择抄送给谁">
            <el-option v-for="item in staffs" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="字段状态">
          ...
          <el-row v-for="(item,$index) in statusChange" :key="item+$index" style="margin-top: 2px">
            <el-col :span="11">
              <el-select v-model="item.name" style="width: 100%" placeholder="请选择字段">
                <el-option
                  v-for="item in customfieldList"
                  :key="item.id"
                  :label="item.field_name"
            ...
            <el-col :span="8">
              <el-select v-model="item.value" style="width: 100%" placeholder="请选择状态">
                <el-option
            ...
            <el-col v-if="$index!==0" :span="2" style="text-align: center">
              <i
...
export default {
  ...
  methods: {
    ...
    handleEdit(scope) {
      ...
      const arr = []
      for (const pro in this.wfstate.state_fields) {
